...
/**
 * uploaderss.php
 * Uploader alterno para CSV de ventas (Puawai) usando Azure SQL (SQL Server) via PDO (pdo_sqlsrv)
 */
...
// Incluir archivo de configuración de base de datos (DEBE exponer $pdo como instancia PDO sqlsrv)
include("db.php");

// Incluir autenticación Firebase (cliente/JS)
include("includes/auth.php");

// Configuración de seguridad
define('MAX_FILE_SIZE', 16 * 1024 * 1024); // 16MB
define('ALLOWED_EXTENSIONS', ['csv']);
define('UPLOAD_DIR', 'uploads/'); // (no se usa aquí, pero se deja por consistencia)
define('EXPECTED_COLUMNS', 60); // El CSV tiene 60 columnas
...
function sanitizeData($data)
{
    ...
    // Decodifica entidades HTML (a veces los CSV vienen con &amp; o &apos;)
    $decoded = html_entity_decode($s, ENT_QUOTES | ENT_HTML5, 'UTF-8');

    // Segundo pase para casos doblemente escapados: &amp;apos;
    if ($decoded !== $s) {
...
// Función para convertir fecha a Y-m-d (o null si no parsea)
        ...
        if (!isset($_FILES['csv_file']) || $_FILES['csv_file']['error'] !== UPLOAD_ERR_OK) {
            ...
            if ($file['size'] > MAX_FILE_SIZE) {
                ...
                if (!in_array($fileExtension, ALLOWED_EXTENSIONS, true)) {
                    ...
                    // Procesar el archivo CSV e insertar en Azure SQL (SQL Server)
                    try {
                        if (!isset($pdo) || !($pdo instanceof PDO)) {
                            throw new Exception('No hay una conexión PDO válida (Azure SQL). Revisa db.php.');
                        }

                        // (Opcional) Asegurar modo estricto de errores
                        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);

                        // Preparar la consulta de inserción (60 columnas - SIN IdSaleTrx si era AUTO_INCREMENT en MySQL)
                        $placeholders = implode(',', array_fill(0, EXPECTED_COLUMNS, '?'));

                        // Nota: En SQL Server, encerrar campos con [] evita problemas con nombres/keywords.
                        $insertQuery = "INSERT INTO [dbo].[sales] (
                        ...
                        // Recomendación: transacción para velocidad (si hay errores por fila, los capturamos y seguimos)
                        ...
                        while (($data = fgetcsv($handle, 100000, ',', '"')) !== false) {
                            ...
                            if ($lineNumber <= 5) {
                                error_log("CSV Sales SS: Saltando fila $lineNumber (metadata): " . implode(',', array_slice($data, 0, 3)));
                                continue;
                            }
                            ...
                            if ($lineNumber === 6) {
                                $headersFound = true;
                                error_log("CSV Sales SS: Encabezados encontrados en fila 6: " . implode(',', array_slice($data, 0, 5)));
                                error_log("CSV Sales SS: Total de columnas en encabezados: " . count($data));
                                continue;
                            }
                            ...
                            // Convertir fechas específicas (por índice)
                            $cleanData[2] = convertDate($cleanData[2]); // PrebookCreatedOn
                            $cleanData[3] = convertDate($cleanData[3]); // InvoiceDate
                            $cleanData[6] = convertDate($cleanData[6]); // FarmShipDate
                            $cleanData[8] = convertDate($cleanData[8]); // TruckDate
                            ...
                            try {
                                ...
                                // Guardar error por fila y continuar
                                $errors[] = "Línea $lineNumber: Error al insertar - " . $ex->getMessage();
                            }
                        }
                        ...
                        try {
                            ...
                            // Si el commit falla, rollback para no dejar transacción abierta
                            if ($pdo->inTransaction()) $pdo->rollBack();
                            throw new Exception("Error al confirmar transacción: " . $commitEx->getMessage());
                        }

                        if ($insertedRows > 0) {
                            $message = "Proceso completado exitosamente en Azure SQL. $insertedRows filas de ventas insertadas.";
                            $messageType = 'success';
                        ...
                        // Si quedó transacción abierta por un error duro, rollback
                        ...
                        $messageType = 'danger';
                        error_log("CSV Sales SS Error: " . $e->getMessage());
                    }
                }
            }
        }
                ...
                <div class="card shadow">
                    <div class="card-header bg-success text-white">
                        <h3 class="card-title mb-0">
                            ...
                            Subir Archivo CSV - Puawai Sales Data (Azure SQL)
                        </h3>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="alert alert-info">
                            ...
                            <ul class="mb-0">
                                ...
                                <li><strong>Estructura esperada:</strong> 5 filas de metadata + 1 fila de encabezados +
                                    datos</li>
                                <li>El archivo debe tener exactamente <strong><?php echo EXPECTED_COLUMNS; ?>
                                        columnas</strong></li>
                                ...
                                <li><strong>Destino:</strong> Azure SQL (SQL Server) tabla <code>dbo.sales</code></li>
                            </ul>
                        </div>
                        ...
                        <form method="POST" enctype="multipart/form-data" id="uploadForm">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text">
                                    ...
                                    Archivo CSV esperado de <?php echo EXPECTED_COLUMNS; ?> columnas (máximo 16MB)
                                </div>
                            </div>
                            ...
                            <div class="mt-auto">
                                <div class="d-grid gap-2">
                                    <button type="submit" class="btn btn-success btn-lg" id="submitBtn">
                                        ...
                                        Subir y Procesar Ventas CSV (SQL Server)
                                    </button>
                        ...
                        <div class="text-muted small">
                            <strong>Nota técnica:</strong> Este uploader usa PDO (pdo_sqlsrv) y carga en Azure SQL.
                            Si la inserción falla con “Invalid object name”, revisa que la tabla exista en
                            <code>dbo</code>
                            y que los nombres de columnas coincidan exactamente.
                        </div>
...
<script>
// Proteger la página (Firebase Auth client-side)
document.addEventListener('DOMContentLoaded', function() {
    protectPage();
});

// UX: progreso "simulado"
document.getElementById('uploadForm').addEventListener('submit', function() {
    const submitBtn = document.getElementById('submitBtn');
    const progressContainer = document.getElementById('progressContainer');

    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Procesando CSV en Azure SQL...';
...
document.getElementById('csv_file').addEventListener('change', function() {
    ...
    formText.innerHTML =
        '<i class="fas fa-check-circle text-success me-1"></i>Archivo CSV válido - se cargará a Azure SQL (SQL Server)';
